...
def create_table_visualization(original_data, reproduced_data, output_dir):
    """Create Table 3 visualization with original and reproduced results."""
    fig, axes = plt.subplots(2, 1, figsize=(14, 8))
    fig.suptitle('Table 3: Average computation time (in seconds) across 1000 replications\n'
    ...
    output_file = os.path.join(output_dir, 'table3_computation_time.png')
    plt.savefig(output_file, dpi=300, bbox_inches='tight', format='png')
    print(f"Table 3 visualization saved to: {output_file}")
    plt.close()
...
def main():
    print("="*70, flush=True)
    print("Table 3: Computation Time Visualization", flush=True)
    print("="*70, flush=True)
